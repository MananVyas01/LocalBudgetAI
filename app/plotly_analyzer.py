...
def create_interactive_bar_chart(
    expense_summary: pd.Series, title: str = "Total Spending by Category"
) -> go.Figure:
...
def create_interactive_pie_chart(
    expense_summary: pd.Series, title: str = "Expense Distribution by Category"
) -> go.Figure:
    ...
    fig.update_layout(
        font=dict(size=12), height=500, margin=dict(t=80, b=50, l=50, r=150)
    )
...
def create_interactive_line_chart(
    monthly_data: pd.Series, title: str = "Monthly Expense Trend"
) -> go.Figure:
...
def create_category_comparison(df: pd.DataFrame, categories: List[str]) -> go.Figure:
    ...
    monthly_by_category = (
        df_copy.groupby(["Month", "Category"])["Amount"].sum().reset_index()
    )
