...
class AdvancedFinancialAI:
    ...
    def __init__(self):
        ...
        self.expense_categories = {
            'essential': ['groceries', 'utilities', 'rent', 'mortgage', 'insurance', 'healthcare'],
    ...
    def analyze_query_intent(self, query: str) -> Dict[str, Any]:
        ...
        if any(word in query_lower for word in ['recommend', 'suggest', 'advice', 'should', 'how to']):
            intent_analysis['recommendation_request'] = True
        
        return intent_analysis

    def generate_advanced_context(self, df: pd.DataFrame, query_intent: Dict[str, Any]) -> str:
        ...
        try:
            ...
            context_parts.append(f"""💰 **Financial Overview**
...
Savings Rate: {(net_cash_flow/total_income*100 if total_income > 0 else 0):.1f}%""")
    ...
    def _analyze_categories(self, expenses_df: pd.DataFrame) -> str:
        ...
        category_summary = expenses_df.groupby('category')['amount'].agg(['sum', 'count', 'mean']).round(2)
        category_summary['percentage'] = (category_summary['sum'] / category_summary['sum'].sum() * 100).round(1)
        ...
        for category in category_summary.index:
            ...
            if any(essential in category.lower() for essential in self.expense_categories['essential']):
                essential_spending += amount
            elif any(lifestyle in category.lower() for lifestyle in self.expense_categories['lifestyle']):
    ...
    def _analyze_temporal_patterns(self, expenses_df: pd.DataFrame) -> str:
        ...
        if len(expenses_df) > 30:
            ...
            if len(monthly_spending) > 1:
                ...
                change = ((monthly_spending.iloc[-1] - monthly_spending.iloc[-2]) / monthly_spending.iloc[-2] * 100)
...
{monthly_trend}"""
    ...
    def _analyze_spending_behavior(self, expenses_df: pd.DataFrame) -> str:
        ...
        cv = std_dev / avg_transaction if avg_transaction > 0 else 0  # Coefficient of variation
        
        consistency = "Consistent" if cv < 0.5 else "Variable" if cv < 1.0 else "Highly Variable"
...
💡 Behavior Insight: {'Your spending is quite predictable.' if cv < 0.5 else 'Your spending varies significantly - consider budgeting for irregular expenses.'}"""
    ...
    def _generate_predictions(self, df: pd.DataFrame) -> str:
        ...
        # Monthly aggregation
        monthly_data = df_copy.groupby(df_copy['date'].dt.to_period('M'))['amount'].sum()
...
📊 Recent 3-Month Pattern: {recent_months.round(2).to_dict()}"""
    ...
    def _generate_budget_insights(self, expenses_df: pd.DataFrame) -> str:
        ...
        category_breakdown = expenses_df.groupby('category')['amount'].sum().sort_values(ascending=False)
        ...
        for category, amount in category_breakdown.head(5).items():
            ...
            if 'food' in category.lower() or 'groceries' in category.lower():
                if percentage > 15:
                    recommendations.append(f"🍽️ Food spending is {percentage:.1f}% (recommended: 10-15%)")
            elif 'entertainment' in category.lower():
                if percentage > 10:
                    recommendations.append(f"🎉 Entertainment is {percentage:.1f}% (recommended: 5-10%)")
            elif 'transport' in category.lower():
                if percentage > 20:
                    recommendations.append(f"🚗 Transportation is {percentage:.1f}% (recommended: 10-20%)")
...
def advanced_query_expense_ai(query: str, df: pd.DataFrame, model_choice: str = 'mistral') -> Tuple[str, Optional[str]]:
    """
    ...
    try:
        ...
        response = ollama.chat(
            ...
            messages=[
                ...
                {"role": "user", "content": f"""Financial Data Analysis:
{advanced_context}
...
def smart_expense_categorization(description: str, amount: float) -> str:
    ...
    category_patterns = {
        'Groceries': ['grocery', 'market', 'food', 'supermarket', 'kroger', 'walmart', 'whole foods', 'trader'],
        'Restaurants': ['restaurant', 'cafe', 'coffee', 'starbucks', 'mcdonald', 'pizza', 'delivery', 'uber eats', 'doordash'],
        ...
        'Healthcare': ['hospital', 'doctor', 'pharmacy', 'medical', 'dental', 'vision', 'cvs', 'walgreens'],
        'Entertainment': ['movie', 'theater', 'netflix', 'spotify', 'game', 'concert', 'event'],
        'Shopping': ['amazon', 'target', 'mall', 'store', 'retail', 'clothing', 'shoes'],
        ...
        'Income': ['salary', 'payroll', 'deposit', 'refund', 'bonus'] if amount > 0 else []
    }
