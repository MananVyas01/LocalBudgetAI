...
class ExpenseDatabase:
    ...
    def insert_expense(self, date: str, amount: float, category: str, 
                      description: str = "") -> int:
        """
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            logger.info(f"Inserted expense {expense_id}: {category} ${amount} on {date}")
            return expense_id
    
    def fetch_expenses(self, limit: Optional[int] = None, 
        ...
        """
        query = "SELECT id, date, amount, category, description, created_at FROM expenses"
        params = []
        conditions = []
    ...
    def update_expense(self, expense_id: int, date: str, amount: float, 
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if updated:
                logger.info(f"Updated expense {expense_id}: {category} ${amount} on {date}")
            else:
    ...
    def import_from_dataframe(self, df: pd.DataFrame) -> int:
        ...
        for _, row in df.iterrows():
            try:
                ...
                if pd.isna(row['amount']) or not isinstance(row['amount'], (int, float)):
                    continue
