...
def analyze_expenses_by_category(
    df: pd.DataFrame, include_income: bool = False, min_amount_threshold: float = 0.01
) -> pd.Series:
    """
    ...
    if not include_income:
        ...
        df_clean["Amount"] = df_clean[
            "Amount"
        ].abs()  # Convert to positive for readability
    ...
    logger.info(
        f"Analyzed {len(result)} expense categories totaling ${result.sum():.2f}"
    )
    return result


def plot_expense_bar_chart(
    expense_summary: pd.Series,
    ...
    bars = expense_summary.plot(
        kind="bar", ax=ax, color=colors, edgecolor="white", linewidth=0.8, alpha=0.8
    )
    ...
    if show_values:
        for i, (category, value) in enumerate(expense_summary.items()):
            ax.text(
                i,
                value + max(expense_summary) * 0.01,
                f"${value:.0f}",
                ha="center",
                va="bottom",
                fontweight="bold",
...
def analyze_monthly_trend(
    df: pd.DataFrame, include_income: bool = False, date_format: Optional[str] = None
) -> pd.Series:
    """
    ...
    if date_format:
        try:
            df_clean["Date_Parsed"] = pd.to_datetime(
                df_clean["Date"], format=date_format
            )
    ...
    logger.info(
        f"Analyzed {len(monthly_totals)} months of data, "
...
def generate_expense_summary_report(df: pd.DataFrame) -> dict:
    ...
    try:
        ...
        top_category = (
            category_summary.index[0] if not category_summary.empty else "N/A"
        )
